...
try:
    from longport.openapi import TradeContext, Config, OpenApiException
except ImportError:
...
def fetch_fund_positions(symbols: Optional[List[str]] = None) -> None:
    """
    获取基金持仓信息
    ...
    try:
        # 检查环境变量
        required_env_vars = ['LONGPORT_APP_KEY', 'LONGPORT_APP_SECRET', 'LONGPORT_ACCESS_TOKEN']
        ...
        for account_idx, account in enumerate(resp.list, 1):
            ...
            for fund_idx, fund in enumerate(account.fund_info, 1):
                ...
                # 计算盈亏（如果数据可用）
                try:
                    current_value = float(fund.current_net_asset_value) if fund.current_net_asset_value else 0
                    cost_value = float(fund.cost_net_asset_value) if fund.cost_net_asset_value else 0
                    holding_units = float(fund.holding_units) if fund.holding_units else 0
