...
try:
    ...
    print(f"错误: 无法导入longport模块。请先安装: pip install longport")
    print(f"详细错误信息: {e}")
    sys.exit(1)
...
def get_stock_positions(symbols: Optional[List[str]] = None) -> None:
    """
    获取股票持仓信息
    ...
    for i, account in enumerate(stock_list, 1):
        ...
        for j, stock_info in enumerate(stock_info_list, 1):
            ...
            try:
                ...
                print(f"    持仓价值: {stock_value:.2f} {stock_info.get('currency', '')}")
