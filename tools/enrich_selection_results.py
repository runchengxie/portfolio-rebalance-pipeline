...
# 这是输入文件
INPUT_FILE = (
    OUTPUTS_DIR / f"point_in_time_backtest_top_{NUM_STOCKS_TO_SELECT}_stocks.xlsx"
)
# 这是输出文件
ENRICHED_OUTPUT_FILE = (
    OUTPUTS_DIR
    / f"point_in_time_backtest_top_{NUM_STOCKS_TO_SELECT}_stocks_enriched.xlsx"
)
...
def load_company_info(data_dir: Path) -> pd.DataFrame:
    ...
    if not company_path.exists():
        print(
            f"[WARNING] Company info file not found in '{data_dir}'. Cannot enrich results."
        )
        return pd.DataFrame()
    ...
    try:
        ...
        df_co_info = df_co[existing_cols].drop_duplicates(
...
def enrich_portfolio_file(
    input_path: Path, output_path: Path, company_info: pd.DataFrame
):
    """读取选股结果，丰富信息，并保存到新文件。"""
    ...
    with pd.ExcelWriter(output_path) as writer:
        for sheet_name, df_portfolio in xls.items():
            if "Ticker" not in df_portfolio.columns:
                print(
                    f"  - Skipping sheet '{sheet_name}' as it has no 'Ticker' column."
                )
