...
def print_summary_report(metrics: dict):
    ...
    print(f'='*50)
    print(f"Time Period Covered:     {metrics['start_date'].strftime('%Y-%m-%d')} to {metrics['end_date'].strftime('%Y-%m-%d')}")
...
def main():
    ...
    portfolio_value.plot(ax=ax, label='SPY Buy & Hold (Total Return)', color='darkorange', lw=2)
    
    ax.set_title(f'SPY Benchmark: Buy-and-Hold Total Return ({START_DATE.year} - {END_DATE.year})', fontsize=16)
