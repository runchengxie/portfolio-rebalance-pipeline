...
class TestPaths:
    ...
    def test_portfolio_file_paths(self):
        ...
        assert AI_PORTFOLIO_FILE == OUTPUTS_DIR / "point_in_time_ai_stock_picks_all_sheets.xlsx"
        assert AI_PORTFOLIO_FILE.suffix == ".xlsx"
        
        # 量化投资组合文件
        assert isinstance(QUANT_PORTFOLIO_FILE, Path)
        assert QUANT_PORTFOLIO_FILE == OUTPUTS_DIR / "point_in_time_backtest_quarterly_sp500_historical.xlsx"
...
class TestStrategyLogger:
    ...
    def test_strategy_logger_with_real_logging(self, tmp_path):
        ...
        with patch('stock_analysis.utils.logging.OUTPUTS_DIR', tmp_path):
            # 创建使用logging的策略日志器
            strategy_logger = StrategyLogger(use_logging=True, logger_name="real_logging_test")
            
            # 设置文件日志
            strategy_logger.logger = setup_logging("real_logging_test", log_file="strategy_test.log")
...
class TestLoggingIntegration:
    ...
    def test_strategy_logger_compatibility(self, tmp_path):
        ...
        with patch('stock_analysis.utils.logging.OUTPUTS_DIR', tmp_path):
            ...
            # 创建策略日志器
            strategy_logger = StrategyLogger(use_logging=True, logger_name="strategy_compat")
            strategy_logger.logger = setup_logging("strategy_compat", log_file="strategy.log")
