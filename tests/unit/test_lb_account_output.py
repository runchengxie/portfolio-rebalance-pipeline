...
class TestLBAccountTableOutput:
    ...
    def test_lb_account_prints_positions_table(self, capsys):
        ...
        fake = SimpleNamespace(
            cash=1234.56,
            positions=[
                {"symbol": "AAPL.US", "qty": 10, "last": 199.99, "est_value": 1999.9}
            ],
        )
        with patch("stock_analysis.cli.LongPortClient") as M:
            ...
            assert (
                cli.run_lb_account(
                    env="test", only_funds=False, only_positions=False, fmt="table"
                )
                == 0
            )
    ...
    def test_table_output_only_funds(self, capsys):
        ...
        with patch("stock_analysis.cli.LongPortClient", return_value=mock_client):
            result = cli.run_lb_account(
                env="test", only_funds=True, only_positions=False, fmt="table"
            )
    ...
    def test_table_output_only_positions(self, capsys):
        ...
        with patch("stock_analysis.cli.LongPortClient", return_value=mock_client):
            result = cli.run_lb_account(
                env="test", only_funds=False, only_positions=True, fmt="table"
            )
    ...
    def test_table_output_real_environment_warning(self, capsys):
        ...
        with patch("stock_analysis.cli.LongPortClient", return_value=mock_client):
            result = cli.run_lb_account(
                env="real", only_funds=True, only_positions=False, fmt="table"
            )
    ...
    def test_table_output_both_environments(self, capsys):
        ...
        with patch("stock_analysis.cli.LongPortClient", return_value=mock_client):
            result = cli.run_lb_account(
                env="both", only_funds=False, only_positions=False, fmt="table"
            )
...
class TestLBAccountJSONOutput:
    ...
    def test_lb_account_outputs_json(self, capsys):
        ...
        with patch("stock_analysis.cli.LongPortClient") as M:
            ...
            assert (
                cli.run_lb_account(
                    env="test", only_funds=True, only_positions=False, fmt="json"
                )
                == 0
            )
    ...
    def test_json_output_structure(self, capsys):
        ...
        with patch("stock_analysis.cli.LongPortClient", return_value=mock_client):
            result = cli.run_lb_account(
                env="test", only_funds=False, only_positions=False, fmt="json"
            )
    ...
    def test_json_output_both_environments(self, capsys):
        ...
        with patch("stock_analysis.cli.LongPortClient", return_value=mock_client):
            result = cli.run_lb_account(
                env="both", only_funds=False, only_positions=False, fmt="json"
            )
...
class TestLBAccountErrorHandling:
    ...
    def test_client_connection_error(self, capsys):
        """测试客户端连接错误"""
        with patch(
            "stock_analysis.cli.LongPortClient",
...
class TestLBAccountParameterValidation:
    ...
    def test_conflicting_flags(self, capsys):
        """测试冲突的标志参数"""
        mock_client = Mock()
        mock_client.portfolio_snapshot.return_value = (1000.0, {"AAPL.US": 10})
        ...
        with patch("stock_analysis.cli.LongPortClient", return_value=mock_client):
            ...
            result = cli.run_lb_account(
                env="test", only_funds=True, only_positions=True, fmt="table"
            )
