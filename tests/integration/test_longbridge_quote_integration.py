...
# 尝试导入longbridge，如果失败则跳过所有测试
longbridge = pytest.importorskip("longbridge")

from stock_analysis.broker.longbridge_client import LongBridgeClient, get_config


def check_longbridge_credentials():
    """检查LongBridge API凭据是否配置。"""
    required_vars = ["LONGBRIDGE_APP_KEY", "LONGBRIDGE_APP_SECRET", "LONGBRIDGE_ACCESS_TOKEN"]
    return all(os.getenv(var) for var in required_vars)


@pytest.mark.integration
@pytest.mark.skipif(
    not check_longbridge_credentials(),
    reason="LongBridge API凭据未配置，跳过集成测试"
)
def test_get_config_from_env():
    """测试从环境变量获取LongBridge配置。"""
    config = get_config()
...
def test_longbridge_client_initialization():
    """测试LongBridgeClient初始化。"""
    try:
        ...
        pytest.fail(f"LongBridgeClient初始化失败: {e}")
