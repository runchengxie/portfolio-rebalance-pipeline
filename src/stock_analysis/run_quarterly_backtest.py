...
def load_all_price_data_from_db(db_path: Path, all_needed_tickers: set, start_date: datetime.date, end_date: datetime.date) -> dict:
    ...
    try:
        ...
        master_dates_df = pd.read_sql_query(date_query, con, params=[start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d')], parse_dates=['Date'])
        ...
        bulk_query = f"SELECT Date, Ticker, Open, High, Low, Close, Volume, Dividend FROM share_prices WHERE Ticker IN ({placeholders}) AND Date >= ? AND Date <= ? ORDER BY Ticker, Date"
        params = tickers_list + [start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d')]
        all_data = pd.read_sql_query(bulk_query, con, params=params, parse_dates=['Date'])

        # --- CORRECCIÓN APLICADA AQUÍ ---
        # Elimina cualquier posible duplicado por Ticker y Fecha, manteniendo la última entrada.
...
def setup_logging():
    log_file = OUTPUTS_DIR / 'backtest_log_quarterly.txt'
    if log_file.exists():
        log_file.unlink()
...
def run_backtest(data_feeds: dict, portfolios: dict, initial_cash: float, start_date: datetime.date, end_date: datetime.date):
    print("\n--- Running Quarterly Point-in-Time Strategy (Total Return) ---")
    cerebro = bt.Cerebro()
    ...
    final_value = initial_cash * cumulative_returns.iloc[-1] if not cumulative_returns.empty else initial_cash
    max_drawdown = strat.analyzers.drawdown.get_analysis().max.drawdown

    duration_in_days = (end_date - start_date).days
    annualized_return = 0.0
    if duration_in_days > 0 and (1 + total_return) > 0:
        duration_in_years = duration_in_days / 365.25
        if duration_in_years > 0:
            annualized_return = ((1 + total_return) ** (1 / duration_in_years)) - 1

    print(f'\n' + '='*50)
    print(f'{"Quarterly Backtest Results (Total Return)":^50}')
    print(f'='*50)
    ...
    portfolio_value = initial_cash * cumulative_returns if not cumulative_returns.empty else pd.Series({pd.Timestamp(start_date): initial_cash, pd.Timestamp(end_date): initial_cash})
    ...
    portfolio_value.plot(ax=ax, label='Quarterly Multi-Factor Strategy', color='royalblue', lw=2)
    
    ax.set_title(f'Quarterly Strategy Backtest Performance ({start_date.year} - {end_date.year})', fontsize=16)
    ...
    output_path = OUTPUTS_DIR / 'quarterly_strategy_cumulative_returns.png'
