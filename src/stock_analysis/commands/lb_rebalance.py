...
def run_lb_rebalance(input_file: str, account: str = "main", dry_run: bool = True, env: str = "test") -> int:
    ...
    try:
        ...
        if env == "real" and dry_run:
            logger.error("拒绝执行：你选择了 real 环境但仍是干跑模式。请加 --execute 再来。")
            return 1
        ...
        try:
            ...
            logger.info(f"成功读取文件，使用 sheet: {sheet_name}，包含 {len(tickers)} 条记录")
        ...
        try:
            # 制定调仓计划
            rebalance_result = rebalance_service.plan_rebalance(tickers, account_snapshot)
            rebalance_result.dry_run = dry_run
            rebalance_result.sheet_name = sheet_name
            
            # 执行订单
            executed_orders = rebalance_service.execute_orders(rebalance_result.orders, dry_run)
