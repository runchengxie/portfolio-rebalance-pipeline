...
这是一个经过重构和优化的Python项目，实现了一个**两阶段**的量化选股策略。第一阶段使用传统的多因子模型进行初步筛选；第二阶段则利用 **Google Gemini Pro** 模型，以后价值投资者的视角对初选列表进行深度分析和精选，最终构建投资组合并进行严格的**“时点”（Point-in-Time）**回测。
...
5. **初步筛选**: 在每个季度，选出滚动平均因子分排名前 **20** 的股票，作为AI分析的候选列表。
...
* `point_in_time_backtest_quarterly_sp500_historical.xlsx`: **[量化初筛结果]** 每个季度筛选出的前20名候选股票。

* `point_in_time_ai_stock_picks_all_sheets.xlsx`: **[AI精选结果]** AI从候选股中精选出的10只股票，包含置信度分数和详细理由。
